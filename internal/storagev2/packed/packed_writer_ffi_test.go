...
func TestGetManifestFieldIDs_InvalidColumnName(t *testing.T) {
	...
	writer, err := NewFFIPackedWriter("files/packed_writer_invalid_column/1", 0, schema, columnGroups, nil, nil)
	...
	fields, err := GetManifestFieldIDs(manifest, CreateStorageConfig())
...
func TestGetManifestFieldIDs_FromPackedWriterManifest(t *testing.T) {
	...
	writer, err := NewFFIPackedWriter("files/packed_writer_field_ids/1", 0, schema, columnGroups, nil, nil)
	require.NoError(t, err)
	...
	_, err = writer.Close()
	require.ErrorContains(t, err, "already closed or destroyed")
}
