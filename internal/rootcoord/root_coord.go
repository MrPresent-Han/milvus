...
type BackfillAtomicGateRegistrar interface {
	...
	// is retried by the ack loop. vectorFieldIDs is the subset of fieldIDs with a vector
	// data type — those additionally hold the round until their index covers every
	// pre-tick sealed segment.
	RegisterWatermark(ctx context.Context, collectionID, roundID int64, fieldIDs, vectorFieldIDs []int64, watermark int32, schemaChangeTimeTick uint64) error
}
