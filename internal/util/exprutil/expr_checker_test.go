...
func TestParseRanges(t *testing.T) {
	...
	// test query plan
	{
		...
		assert.Equal(t, range0.left.Val.(*planpb.GenericValue_Int64Val).Int64Val, int64(50))
		assert.Nil(t, range0.right)
		assert.Equal(t, range0.includeLeft, false)
		assert.Equal(t, range0.includeRight, false)
	}
	{
		expr := "cluster_key_field > 50 && cluster_key_field <= 100"
		queryPlan, err := planparserv2.CreateRetrievePlan(schemaHelper, expr)
		assert.NoError(t, err)
		planExpr, err := ParseExprFromPlan(queryPlan)
		assert.NoError(t, err)
		parsedRanges, matchALL := ParseRanges(planExpr, ClusteringKey)
		assert.False(t, matchALL)
		assert.Equal(t, 2, len(parsedRanges))
		range0 := parsedRanges[0]
		assert.Equal(t, range0.left.Val.(*planpb.GenericValue_Int64Val).Int64Val, int64(50))
		assert.Nil(t, range0.right)
		assert.Equal(t, range0.includeLeft, false)
		...
		range1 := parsedRanges[1]
		assert.Nil(t, range1.left, nil)
		assert.Equal(t, range1.right.Val.(*planpb.GenericValue_Int64Val).Int64Val, int64(100))
		assert.Equal(t, range1.includeLeft, false)
		assert.Equal(t, range1.includeRight, true)
	}
	{
		expr := "cluster_key_field > 50 && varChar_field <= 'aaa'"
		queryPlan, err := planparserv2.CreateRetrievePlan(schemaHelper, expr)
		assert.NoError(t, err)
		planExpr, err := ParseExprFromPlan(queryPlan)
		assert.NoError(t, err)
	...
	}
	{
		...
		assert.Equal(t, 0, len(parsedRanges))
	}
	{
		expr := "cluster_key_field > 50 && cluster_key_field <= 100 && cluster_key_field<=180"
		queryPlan, err := planparserv2.CreateRetrievePlan(schemaHelper, expr)
		assert.NoError(t, err)
		planExpr, err := ParseExprFromPlan(queryPlan)
		assert.NoError(t, err)
		parsedRanges, matchALL := ParseRanges(planExpr, ClusteringKey)
		assert.False(t, matchALL)
		assert.Equal(t, 3, len(parsedRanges))
	}
}
