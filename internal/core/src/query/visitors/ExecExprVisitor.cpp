...
ExecExprVisitor::InChunkRange(InRangeFunc inRange,
    ...
    if (!fieldChunkMetrics.hasValue_)
        return true;
    T minValue;
    T maxValue;
    switch (dataType) {
...
ExecExprVisitor::ExecUnaryRangeVisitorDispatcherImpl(UnaryRangeExpr& expr_raw)
    ...
    auto default_range_func = [&](MayConstRef<T> min, MayConstRef<T> max) {
        return true;
    };
    switch (op) {
        case OpType::Equal: {
            ...
            return ExecRangeVisitorImpl<T>(
                field_id, index_func, elem_func, in_range_func);
        }
...
#define UnaryRangeJSONCompareNotEqual(cmp)                    \
    ...
    auto default_range_func = [&](const milvus::Json& min,
                                  const milvus::Json& max) { return true; };
    switch (op) {
        case OpType::Equal: {
...
ExecExprVisitor::ExecUnaryRangeVisitorDispatcherArray(UnaryRangeExpr& expr_raw)
    ...
    using GetType =
        std::conditional_t<std::is_same_v<ExprValueType, std::string>,
                           std::string_view,
                           ExprValueType>;
    auto default_range_func = [&](const milvus::ArrayView& min,
                                  const milvus::ArrayView& max) {
        return true;
    };
    switch (op) {
        case OpType::Equal: {
...
ExecExprVisitor::ExecBinaryRangeVisitorDispatcher(BinaryRangeExpr& expr_raw)
    ...
    if (lower_inclusive && upper_inclusive) {
        ...
        auto in_range_func = [val1, val2](MayConstRef<T> min,
                                          MayConstRef<T> max) {
...
ExecExprVisitor::ExecBinaryRangeVisitorDispatcherJson(BinaryRangeExpr& expr_raw)
    ...
    // no json index now
    auto index_func = [=](Index* index) { return TargetBitmap{}; };
    auto default_in_range_func = [&](const milvus::Json& min,
                                     const milvus::Json& max) { return true; };
...
ExecExprVisitor::ExecBinaryRangeVisitorDispatcherArray(
    ...
    // no json index now
    auto index_func = [=](Index* index) { return TargetBitmap{}; };
    auto default_in_range_func = [&](const milvus::ArrayView& min,
                                     const milvus::ArrayView& max) {
        return true;
    };
...
ExecExprVisitor::ExecTermArrayFieldInVariable(TermExpr& expr_raw)
    ...
    auto& expr = static_cast<TermExprImpl<ExprValueType>&>(expr_raw);
    auto index_func = [](Index* index) { return TargetBitmap{}; };
    auto in_range_func = [&](const milvus::ArrayView& min,
                             const milvus::ArrayView& max) { return true; };
    int index = -1;
    if (expr.column_.nested_path.size() > 0) {
        index = std::stoi(expr.column_.nested_path[0]);
    }
...
ExecExprVisitor::visit(ExistsExpr& expr) {
    ...
    switch (expr.column_.data_type) {
        case DataType::JSON: {
            ...
            auto in_range_func = [&](const milvus::Json& min,
                                     const milvus::Json& max) { return true; };
