...
// Helper to compare GroupByValueType (optional<variant>)
int
CompareGroupByValue(const GroupByValueType& lhs, const GroupByValueType& rhs) {
...
SearchOrderBy(milvus::OpContext* op_ctx,
              const std::vector<plan::SearchOrderByNode::OrderByField>&
                  order_by_fields,
    ...
    using GetterFunc = std::function<GroupByValueType(int64_t)>;
    ...
    for (const auto& field : order_by_fields) {
        ...
        switch (data_type) {
            case DataType::BOOL: {
                ...
                getter = [dg](int64_t idx) -> GroupByValueType {
                    auto val = dg->Get(idx);
                    if (!val.has_value()) {
                        return std::nullopt;
                    }
                    return std::make_optional(std::variant<std::monostate, int8_t, int16_t, int32_t, int64_t, bool, std::string>(val.value()));
                };
                break;
            }
            case DataType::INT8: {
            ...
            case DataType::DOUBLE: {
                // TODO: Support float/double order by
                // Currently not supported as GroupByValueType doesn't include float/double
                ThrowInfo(UnexpectedError,
                          "Order by field type {} not yet supported (float/double)",
                          data_type);
                break;
            }
            ...
            case DataType::JSON: {
                // For JSON, use GroupByValueType directly
                auto dg = GetDataGetter<GroupByValueType>(
                    op_ctx, segment, field.field_id_, field.json_path_);
                getter = [dg](int64_t idx) -> GroupByValueType {
                    return dg->Get(idx);
                };
    ...
    auto comparator = [&](size_t lhs_idx, size_t rhs_idx) -> bool {
        for (size_t field_idx = 0; field_idx < order_by_fields.size();
            ...
            // Compare values using GroupByValueType comparison
            int cmp = CompareGroupByValue(lhs_val, rhs_val);
            if (cmp < 0) {
    ...
    if (has_group_by) {
        ...
        for (size_t i = 0; i < topk_per_nq_prefix_sum.size() - 1; ++i) {
            ...
            struct GroupInfo {
                ...
                std::vector<GroupByValueType> first_item_order_by_values;
            };
