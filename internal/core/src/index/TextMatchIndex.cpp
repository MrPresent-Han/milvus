...
TextMatchIndex::Load(const Config& config) {
    ...
    if (it != files_value.end()) {
        ...
        auto index_datas = mem_file_manager_->LoadIndexToMemory(file, config[milvus::THREAD_POOL_PRIORITY]);
    ...
    disk_file_manager_->CacheTextLogToDisk(files_value, config[milvus::THREAD_POOL_PRIORITY]);
    AssertInfo(
