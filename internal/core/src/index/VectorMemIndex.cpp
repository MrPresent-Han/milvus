...
VectorMemIndex<T>::Load(milvus::tracer::TraceContext ctx,
    ...
    // start read file span with active scope
    {
        ...
        if (!slice_meta_filepath
            ...
            auto result =
                file_manager_->LoadIndexToMemory({slice_meta_filepath}, config[milvus::THREAD_POOL_PRIORITY]);
            ...
            for (auto& item : meta_data[META]) {
                ...
                auto batch_data = file_manager_->LoadIndexToMemory(batch, config[milvus::THREAD_POOL_PRIORITY]);
        ...
        if (!pending_index_files.empty()) {
            auto result =
                file_manager_->LoadIndexToMemory(std::vector<std::string>(
                    pending_index_files.begin(), pending_index_files.end()), config[milvus::THREAD_POOL_PRIORITY]);
...
void VectorMemIndex<T>::LoadFromFile(const Config& config) {
    ...
    if (!slice_meta_filepath
        ...
        auto result = file_manager_->LoadIndexToMemory({slice_meta_filepath}, config[milvus::THREAD_POOL_PRIORITY]);
        auto raw_slice_meta = result[INDEX_FILE_SLICE_META];
        Config meta_data = Config::parse(
            std::string(static_cast<const char*>(raw_slice_meta->Data()),
                        raw_slice_meta->Size()));
