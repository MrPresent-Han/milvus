...
InvertedIndexTantivy<T>::Load(milvus::tracer::TraceContext ctx,
    ...
    if (auto it = find_file(INDEX_FILE_SLICE_META);
        ...
        auto index_datas =
            mem_file_manager_->LoadIndexToMemory(null_offset_files, config[milvus::LOAD_PRIORITY]);
        ...
        null_offset_files.push_back(*it);
        auto index_datas = mem_file_manager_->LoadIndexToMemory({*it}, config[milvus::LOAD_PRIORITY]);
    ...
    inverted_index_files.erase(
        ...
        inverted_index_files.end());
    disk_file_manager_->CacheIndexToDisk(inverted_index_files, config[milvus::LOAD_PRIORITY]);
    path_ = prefix;
