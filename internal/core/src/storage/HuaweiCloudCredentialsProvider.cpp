...
HuaweiCloudSTSAssumeRoleWebIdentityCredentialsProvider::Reload() {
    ...
    try {
        result = m_client->GetAssumeRoleWithWebIdentityCredentials(request);
    } catch (const std::exception& e) {
        AWS_LOGSTREAM_ERROR(STS_ASSUME_ROLE_WEB_IDENTITY_LOG_TAG,
                            "Exception during STS credential retrieval: "
                                << e.what()
                                << ". Retaining existing credentials.");
        m_lastReloadFailed = true;
        m_lastFailedReloadTime = std::chrono::steady_clock::now();
        return;
    } catch (...) {
        AWS_LOGSTREAM_ERROR(STS_ASSUME_ROLE_WEB_IDENTITY_LOG_TAG,
                            "Unknown exception during STS credential retrieval."
                            " Retaining existing credentials.");
        m_lastReloadFailed = true;
        m_lastFailedReloadTime = std::chrono::steady_clock::now();
        return;
    ...
    if (!result.success) {
        AWS_LOGSTREAM_WARN(STS_ASSUME_ROLE_WEB_IDENTITY_LOG_TAG,
                           "STS credential retrieval returned failure."
                           " Retaining existing credentials.");
        m_lastReloadFailed = true;
        m_lastFailedReloadTime = std::chrono::steady_clock::now();
        return;
    ...
    if (result.creds.GetAWSAccessKeyId().empty() ||
        result.creds.GetAWSSecretKey().empty()) {
        AWS_LOGSTREAM_WARN(STS_ASSUME_ROLE_WEB_IDENTITY_LOG_TAG,
                           "STS returned empty access key or secret key."
                           " Retaining existing credentials.");
        m_lastReloadFailed = true;
        m_lastFailedReloadTime = std::chrono::steady_clock::now();
        return;
