...
DiskFileManagerImpl::CacheIndexToDiskInternal(
    ...
    for (auto& slices : index_slices) {
        ...
        auto appendIndexFiles = [&]() {
            auto index_chunks = GetObjectData(rcm_.get(), batch_remote_files, priority);
...
std::string
DiskFileManagerImpl::CacheRawDataToDisk(std::vector<std::string> remote_files, milvus::ThreadPoolPriority priority) {
...
DiskFileManagerImpl::CacheRawDataToDisk<float>(
    std::vector<std::string> remote_files, milvus::ThreadPoolPriority priority);
template std::string
