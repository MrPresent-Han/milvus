...
class DiskFileManagerImpl : public FileManagerImpl {
 ...
 public:
    ...
    std::string
    CacheRawDataToDisk(std::vector<std::string> remote_files, milvus::ThreadPoolPriority priority);
 ...
 private:
    ...
    CacheIndexToDiskInternal(
        const std::vector<std::string>& remote_files,
        const std::function<std::string()>& get_local_index_prefix, milvus::ThreadPoolPriority priority) noexcept;
