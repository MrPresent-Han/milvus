...
struct SearchResultPair {
    ...
    std::optional<std::vector<milvus::GroupByValueType>>
...
struct SearchResultPairComparator {
    std::optional<std::vector<plan::SearchOrderByNode::OrderByField>>
        order_by_fields_;
    bool has_order_by_;
    ...
    explicit SearchResultPairComparator(
        const std::optional<std::vector<plan::SearchOrderByNode::OrderByField>>&
            order_by_fields)
    ...
    operator()(const SearchResultPair* lhs, const SearchResultPair* rhs) const {
        if (has_order_by_ && lhs->order_by_values_.has_value() &&
            ...
            for (size_t i = 0; i < fields.size() && i < lhs_vals.size() &&
                ...
                // Compare values
                int cmp = SearchResultPairComparator::CompareGroupByValue(lhs_val, rhs_val);
                if (cmp < 0) {
