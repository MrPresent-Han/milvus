...
MergeReduceHelper::ReduceResultData() {
    ...
    for (int64_t slice_index = 0; slice_index < num_slices_; slice_index++){
        ...
        auto nq_end = slice_nqs_prefix_sum_[slice_index + 1];
        for(int64_t qi = nq_begin; qi < nq_end; qi++){
            StreamReduceSearchResultForOneNQ(qi, slice_topKs_[slice_index]);
        }
    }
...
MergeReduceHelper::FillEntryData() {
    for (auto search_result : search_results_) {
        ...
        for(auto target_field_id: plan_->target_entries_){
            auto field_data = search_result->output_fields_data_.find(target_field_id);
            if(field_data==search_result->output_fields_data_.end()){
                segment->FillFieldEntry(target_field_id, *search_result);
            }
        }
    }
}
