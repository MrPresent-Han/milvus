...
func ReduceSearchResults(ctx context.Context,
	...
	reduceInfo *ReduceInfo) (*internalpb.SearchResults, error) {
	...
	reducedResultData, err := searchReduce.ReduceSearchResultData(ctx, searchResultData, reduceInfo)
	if err != nil {
		log.Warn("shard leader reduce errors", zap.Error(err))
		return nil, err
	}
	searchResults, err := segments.EncodeSearchResultData(ctx, reducedResultData, reduceInfo.nq, reduceInfo.topK, metricType)
	if err != nil {
