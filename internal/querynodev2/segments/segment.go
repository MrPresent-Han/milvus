...
func (s *LocalSegment) warmupCache(ctx context.Context, fieldID int64, method string) (any, error) {
	...
	if err := HandleCStatus(ctx, &status, ""); err != nil {
		log.Warn("warming up chunk cache failed", zap.Error(err), zap.String("method", method))
		return nil, err
	}
	warmUpDuration := tr.RecordSpan()
	log.Info("warming up chunk cache synchronously done", zap.Duration("warmup_duration", warmUpDuration))
	return nil, nil
}
