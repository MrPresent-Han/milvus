...
func (c *SearchAggregationComputer) computeForQi(qi int64) ([]*AggBucketResult, error) {
...
func (c *SearchAggregationComputer) computeLevel(levelIdx int, rows []reduce.RowRef) ([]*AggBucketResult, error) {
	...
	for _, bucket := range keyOrder {
		...
		if !isLeaf {
			subBuckets, err := c.computeLevel(levelIdx+1, bucket.rows)
			if err != nil {
				return nil, err
			}
			result.SubGroups = subBuckets
		}
...
func (c *SearchAggregationComputer) compareRowsForTopHits(a, b reduce.RowRef, sortCriteria []SortCriterion) (int, error) {
	...
	pkA := typeutil.GetPK(c.data.GetIds(), int64(a.RowIdx))
	pkB := typeutil.GetPK(c.data.GetIds(), int64(b.RowIdx))
...
// updateMetrics delegates per-alias accumulation to internal/agg. For each
// compiled metricPlan we read the source value once, wrap it into the
...
func (c *SearchAggregationComputer) updateMetrics(bucket *bucketState, ref reduce.RowRef, plans []metricPlan) error {
	...
	for _, plan := range plans {
		...
		inputs, err := metricAggregateInputs(plan, raw)
		if err != nil {
			return err
		}
		for i, aggregate := range plan.aggregates {
			if err := aggregate.Update(targets[i], inputs[i]); err != nil {
...
// metricAggregateInputs builds the per-aggregate FieldValue slice for one row.
// count → int64(1); sum/min/max → the raw source value; avg expands into
// [raw_value_for_sum, int64(1)_for_count] matching agg.NewAggregate's
// internal layout of avg = [SumAggregate, CountAggregate].
func metricAggregateInputs(plan metricPlan, raw any) ([]*agg.FieldValue, error) {
	one := agg.NewFieldValue(int64(1))
	switch plan.spec.Op {
	case "count":
		return []*agg.FieldValue{one}, nil
	case "sum", "min", "max":
		return []*agg.FieldValue{agg.NewFieldValue(raw)}, nil
	case "avg":
		return []*agg.FieldValue{agg.NewFieldValue(raw), one}, nil
	default:
		return nil, fmt.Errorf("unsupported metric op %q", plan.spec.Op)
	}
}
...
type bucketState struct {
	key   []any
	count int64
	// metricStates maps alias → per-aggregate FieldValue slot. avg's slot has
...
func newBucketState(key []any, plans []metricPlan) *bucketState {
	...
	for _, plan := range plans {
		slots := make([]*agg.FieldValue, len(plan.aggregates))
		for i := range plan.aggregates {
			slots[i] = agg.NewNullFieldValue()
		}
		state.metricStates[plan.alias] = slots
	}
	return state
}

// finalizeMetrics reads out per-alias accumulated values. Avg combines the
// Sum slot and the Count slot into a float64 ratio; other ops return the
// underlying FieldValue.Value() as-is so non-numeric results (string min/max)
// survive without lossy coercion. Null accumulators — e.g. every source row
// was null — yield nil so callers can treat them distinctly from zero.
func finalizeMetrics(plans []metricPlan, states map[string][]*agg.FieldValue) (map[string]any, error) {
	...
	for _, plan := range plans {
		...
		if len(slots) != len(plan.aggregates) {
			return nil, fmt.Errorf("metric %q: accumulator width mismatch", plan.alias)
		}
		switch plan.spec.Op {
		case "avg":
			sum := slots[0]
			...
			if sum.IsNull() || cnt.IsNull() {
				metrics[plan.alias] = nil
				continue
			}
			sumF, err := aggValueToFloat64(sum.Value())
			if err != nil {
				return nil, fmt.Errorf("metric %q: %w", plan.alias, err)
			}
			cntF, err := aggValueToFloat64(cnt.Value())
			if err != nil {
				return nil, fmt.Errorf("metric %q: %w", plan.alias, err)
			}
			if cntF == 0 {
...
// aggValueToFloat64 normalizes an agg.FieldValue.Value() into float64 for the
// avg division step. internal/agg keeps sum/count values in their original
// numeric types (int64, float32, etc.), so avg needs a local widener.
func aggValueToFloat64(v any) (float64, error) {
	switch value := v.(type) {
	case int:
...
func compareValues(a, b any) (int, error) {
	...
	if af, err := compareAsFloat64(a); err == nil {
		bf, err := compareAsFloat64(b)
		if err != nil {
...
// compareAsFloat64 widens any numeric type into float64 for ordering purposes.
// Intentionally separate from the metric-finalization path's narrower helper
// so the order-by comparator keeps working for bool and the full uint family.
func compareAsFloat64(v any) (float64, error) {
	switch value := v.(type) {
	case int:
		return float64(value), nil
	case int8:
