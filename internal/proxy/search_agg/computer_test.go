...
func TestSearchAggregationComputerMultiLevelNested(t *testing.T) {
	...
	require.Len(t, bucketA.SubGroups, 2)

	aBlue := bucketA.SubGroups[0]
	...
	// configured on leaf TopHits, so order follows row insertion.
	...
	require.Len(t, bucketB.SubGroups, 1)
	bRed := bucketB.SubGroups[0]
