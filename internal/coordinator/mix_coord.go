...
func (s *mixCoordImpl) initInternal() error {
	...
	// site (rootcoord DDL ack callback). mixCoordImpl itself is the ProxyGatePusher
	// (PushGatedFields below). The readiness provider + observer are built after the
	// sub-coords are up (datacoord meta must be ready).
	...
	// (DataView = datacoord meta write-side checks: schema watermark, pre-tick growing
	// segments, vector-field index coverage) + observer, reload persisted rounds
	// (restart recovery), and start the periodic revoke sweep. mixCoord owns Start/Stop.
	readiness := NewReadinessProvider(s.datacoordServer)
	// A dropped collection stops compacting and indexing, so stale segments lingering
	// in meta until GC would hold its rounds indefinitely -- the observer force-revokes
	// them. Existence is answered by the first source (rootcoord meta, in process, by
	// ID); only a positive not-found verdict counts as dropped -- any other failure
	// (not healthy, scheduler down) keeps the round for the next sweep, fail-closed.
...
func (s *mixCoordImpl) PushGatedFields(ctx context.Context, collectionID int64, fieldIDs []int64) error {
	// rootcoord surfaces proxy fan-out failures as a Status with a nil Go error, so the
	// returned Status must be checked — otherwise a Revoke would count a missed proxy as
	// released and never retry the push.
	resp, err := s.InvalidateCollectionMetaCache(ctx, &proxypb.InvalidateCollMetaCacheRequest{
		CollectionID:       collectionID,
		DefenceUpdate:      true,
