...
// quiet; revoke latency is dominated by the backfill compaction and index build anyway.
...
func (o *BackfillAtomicGateObserver) sweep() {
	for _, round := range o.registry.List() {
		// A dropped collection stops compacting and indexing, so stale segments linger
		// in meta until GC and its rounds may never satisfy readiness -- force-revoke
		// them here or they leak (etcd + memory + a pointless RepushAll entry) forever.
