...
// advances schema_version, so no segment list needs to be stored. The only scope kind.
const ScopeWatermark BackfillScopeKind = iota

// BackfillScope expresses the round's write-side completion semantics.
type BackfillScope struct {
	Kind      BackfillScopeKind `json:"kind"`
	Watermark int32             `json:"watermark,omitempty"` // schema_version V
	// SchemaChangeTimeTick is the WAL timetick of the schema-change DDL (max across its
	// vchannel appends). The readiness time axis: growing segments born before it may
	// hold pre-V rows, and sealed segments born before it need the vector index (their
	// vector column was backfilled, not computed on ingest). Zero disables both cuts.
	SchemaChangeTimeTick uint64 `json:"schemaChangeTimeTick,omitempty"`
	// VectorFields is the subset of the round's fields that are vector types — those
	// additionally require a Finished index on every pre-tick sealed segment before the
	// round may serve.
	VectorFields []int64 `json:"vectorFields,omitempty"`
}
...
func (r *BackfillAtomicGate) Register(ctx context.Context, round *BackfillRound) error {
	...
	// Idempotency: a round carrying a Source is keyed by that source. A retried or
	// CONCURRENT registration must reuse the existing roundID rather than create a
	// duplicate gate (the caller-allocated roundID is unique per call, but the SAME
	// backfill round may be registered more than once by ack retries). The write lock
	// spans the whole check-and-save: registration is low-frequency, so holding it
	// across the etcd Save only briefly delays a concurrent sweep, in exchange for an
	// atomic source-dedup.
	r.mu.Lock()
	if round.Source != "" {
...
func (r *BackfillAtomicGate) Reload(ctx context.Context) error {
	...
	for _, v := range values {
		...
		// A non-watermark round (legacy external format, feature unreleased) can never
		// be certified by the readiness check — drop it instead of re-arming an
		// un-revokable gate. Note this releases the round's fields immediately.
		if round.Scope.Kind != ScopeWatermark {
			mlog.Warn(ctx, "dropping bump_defence round with unsupported scope kind",
				mlog.FieldCollectionID(round.CollectionID),
				mlog.Int64("roundID", round.RoundID),
				mlog.String("source", round.Source))
			if err := r.metaKV.Remove(ctx, backfillRoundKey(round.CollectionID, round.RoundID)); err != nil {
				mlog.Warn(ctx, "failed to delete unsupported bump_defence round entry", mlog.Err(err))
			}
			continue
		}
		if r.rounds[round.CollectionID] == nil {
