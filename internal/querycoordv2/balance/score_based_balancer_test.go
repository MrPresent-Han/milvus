...
func (suite *ScoreBasedBalancerTestSuite) TestBalanceOneRound() {
	...
	for _, c := range cases {
		suite.Run(c.name, func() {
			...
			suite.broker.EXPECT().GetRecoveryInfo(mock.Anything, c.collectionID, c.replicaID).Return(nil, c.collectionsSegments, nil)
...
func (suite *ScoreBasedBalancerTestSuite) TestStoppedBalance() {
	...
	for i, c := range cases {
		suite.Run(c.name, func() {
			...
			//1. set up target for multi collections
			collection := utils.CreateTestCollection(c.collectionID, int32(c.replicaID))
			suite.broker.EXPECT().GetRecoveryInfo(mock.Anything, c.collectionID, c.replicaID).Return(
