...
// may come from different replica group. We only nceed these shards to form a replica that serves query
// requests.
...
func (s *Server) getCollectionSegmentInfo(collection int64) []*querypb.SegmentInfo {
	...
	for _, segment := range segments {
		if _, existCurrentTarget := currentTargetSegmentsMap[segment.GetID()]; !existCurrentTarget {
			...
			log.Info("hc--- filtered segment being in the intermediate status",
