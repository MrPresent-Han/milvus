...
func (dh *distHandler) updateSegmentsDistribution(resp *querypb.GetDataDistributionResponse) {
	...
	//////
	loadingSegmentIDs := make([]int64, 0, len(loadingSegments))
	for _, segment := range loadingSegments {
		loadingSegmentIDs = append(loadingSegmentIDs, segment.GetID())
	}
	log.Info("hc---", zap.Int64s("loading segmentIDs", loadingSegmentIDs))
	///////
