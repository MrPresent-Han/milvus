...
func (c *Client) AddCollectionFunctionField(ctx context.Context, in *milvuspb.AddCollectionFunctionFieldRequest, opts ...grpc.CallOption) (*milvuspb.AddCollectionFunctionFieldResponse, error) {
	in = typeutil.Clone(in)
	commonpbutil.UpdateMsgBase(
		in.GetBase(),
		commonpbutil.FillMsgBaseFromClient(paramtable.GetNodeID(), commonpbutil.WithTargetID(c.grpcClient.GetNodeID())),
	)
	return wrapGrpcCall(ctx, c, func(client MixCoordClient) (*milvuspb.AddCollectionFunctionFieldResponse, error) {
		return client.AddCollectionFunctionField(ctx, in)
	})
}
