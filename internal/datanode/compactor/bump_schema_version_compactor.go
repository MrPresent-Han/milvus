...
func (t *bumpSchemaVersionCompactionTask) runFullSchemaRewrite(existingFields map[int64]struct{}) (*datapb.CompactionPlanResult, error) {
	// Ref-mode analyzer file resources are attached to the plan by DataCoord
	// best-effort (bump-only and partial backfill never need them). This path
	// does — at createTextIndex, after the whole segment has been read,
	// materialized and rewritten. When they are known missing, fail before any
	// of that work instead.
	if fileresource.GlobalFileManager != nil &&
		fileresource.GlobalFileManager.Mode() == fileresource.RefMode &&
		schemaNeedsTextIndex(t.plan.GetSchema()) &&
		len(t.plan.GetSchema().GetFileResourceIds()) > 0 &&
		len(t.plan.GetFileResources()) == 0 {
		return nil, merr.WrapErrServiceInternalMsg("schema bump full rewrite requires analyzer file resources %v in ref mode but the plan carries none",
			t.plan.GetSchema().GetFileResourceIds())
	}
