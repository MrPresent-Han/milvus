...
// Ref-mode analyzer resources are consumed by createTextIndex only after the
// whole segment has been rewritten; when the plan is known to be missing them,
// the full rewrite must fail before any segment access or I/O.
func (s *BumpSchemaVersionCompactionTaskSuite) TestFullRewriteFailsFastWhenRefModeResourcesMissing() {
	...
	_, err := s.task.runFullSchemaRewrite(map[int64]struct{}{100: {}, 101: {}})
	s.ErrorContains(err, "the plan carries none")
}
