...
func (p *HuaweiCredentialProvider) initClients() error {
	...
	if err != nil {
		log.Warn("HuaweiCloud credential provider: invalid region",
			zap.String("configured_region", regionName), zap.Error(err))
		return errors.Wrapf(err, "invalid HUAWEICLOUD_SDK_REGION: %s", regionName)
	}
