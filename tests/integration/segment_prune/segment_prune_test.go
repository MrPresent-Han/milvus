...
func (s *SegmentPruneSuite) TestPruneByStrField() {
	...
	clusteringColumn := integration.NewVarCharRandomFieldData(clusterKeyFieldName, rowNum, "test_segment_prune", 0, rowNum)
	...
	expr := fmt.Sprintf("%s > \"%s\"", integration.VarCharField, prefix+"99")
	nq := 10
	topk := 10
	roundDecimal := -1
